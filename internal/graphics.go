...
func (impl *Graphics) Clear() {
	displayWidth, displayHeight := impl.window.GetFramebufferSize()
	gl.Viewport(0, 0, int32(displayWidth), int32(displayHeight))
	gl.ClearColor(clearColor.X, clearColor.Y, clearColor.Z, clearColor.W)
	gl.Clear(gl.COLOR_BUFFER_BIT)
}

func NewGraphics(window *glfw.Window) *Graphics {
	impl := &Graphics{
		window:      window,
...
func (impl *Graphics) createDeviceObjects() {
	// Backup GL state
...
func (impl *Graphics) Render(drawData imgui.DrawData) {
...
func (impl *Graphics) Shutdown() {
	impl.invalidateDeviceObjects()
}

func (impl *Graphics) invalidateDeviceObjects() {
	if impl.vboHandle != 0 {
...
func (impl *Graphics) installCallbacks() {
...
func (impl *Graphics) mouseButtonChange(window *glfw.Window, rawButton glfw.MouseButton, action glfw.Action, mods glfw.ModifierKey) {
...
func (impl *Graphics) mouseScrollChange(window *glfw.Window, x, y float64) {
	io := imgui.CurrentIO()
	io.AddMouseWheelDelta(float32(x), float32(y))
}

func (impl *Graphics) keyChange(window *glfw.Window, key glfw.Key, scancode int, action glfw.Action, mods glfw.ModifierKey) {
	io := imgui.CurrentIO()
...
func (impl *Graphics) charChange(window *glfw.Window, char rune) {
	io := imgui.CurrentIO()
