...
void game::action_host_game()
{
    ...
    // check for error
    if (!response) {
        log_trace("could not host game %s", error.c_str());
    ...
    log_trace("waiting for connections");
...
void game::action_join_online_game()
{
    ...
    // check for error
    if (!result) {
        log_trace("could not join game %s", error.c_str());
...
void game::action_add_network_player(const shared_ptr<player> &player)
{
    ...
    log_trace("adding network player %s (%s)", player->name().c_str(), player->id().c_str());
...
{
    log_trace("action network player joined");
...
void game::action_network_player_left(const shared_ptr<player> &p)
{
    ...
    if (players_.size() == 1) {
        ...
        log_trace("still have %zu players", players_.size());
    }
...
{
    if (players_.size() == 1) {
        display_alert(4000, "Game over.", nullptr, [&]() { set_state(&game::state_game_menu); });
        return;
    }
    ...
    if (tieGame) {
        display_alert(4000, "Tie game!", nullptr, [&]() { set_state(&game::state_game_menu); });
    } else if (winner == this_player()) {
        display_alert(4000, "You win!", nullptr, [&]() { set_state(&game::state_game_menu); });
    } else {
        char buf[BUFSIZ + 1] = {0};
        snprintf(buf, BUFSIZ, "%s wins with %u points!", winner->name().c_str(), winner->score().total_score());
        display_alert(4000, buf, nullptr, [&]() { set_state(&game::state_game_menu); });
    }
}
